...
class Suavisado:
    def __init__(self):
        ...
        self.ventana.geometry('400x220+700+200')
    ...
    def suavisar(self):
        ...
        if not len(self.tabla.get_children()):
            ...
            for i in self.tabla.get_children():
                self.tabla.delete(i)
            for i in range(maximo):
                filtro=self.valores[-(orden-1):]+[0]*(orden-1)
                auxiliar=(self.y[i]+sum(filtro))/orden
    ...
    def constructores(self):
        ...
        lorden.place(x=10, y=420)
        self.orden.place(x=70, y=420)
        lmaximo.place(x=180, y=420)
        self.maximo.place(x=250, y=420)
